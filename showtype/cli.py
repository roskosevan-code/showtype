"""Command-line entry point for the Show Type.

    python -m showtype init-db          # create the DB and seed the 8 axes
    python -m showtype axes              # list the seeded axes
    python -m showtype backfill          # load docs/baseline-scores.csv into the DB
    python -m showtype score "The Wire"  # score one show via the Claude API
    python -m showtype score-all --file shows.txt --skip-existing   # sequential
    python -m showtype score-batch --file shows.txt --skip-existing  # Batches API (50% cost)
    python -m showtype similar "The Wire"           # nearest shows in taste-space
    python -m showtype query --where "sweep>=8" --where "register<=4"   # filter by profile
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="showtype", description=__doc__)
    p.add_argument(
